...
const ROOM_SPAWN_DATA := {
	...
	Vector2i(-1,-1): Vector2i(-60, -30), # Tutorial 2
...
const ROOM_SECTION_DATA := {
	...
	Vector2i(-2, -1): {"title": "Puzzle1", "mechanisms": 1, "spawn_point": Vector2i(-550, -230)},
...
func get_new_room_data():
	# Gets the data for the room (title, mechanisms, sector spawn point)
	...
	if is_section_overheating[current_section_data.title] != main_script.is_timer_active:
		main_script.toggle_timer(int(is_section_overheating[current_section_data.title]), 60, Color.RED, main_script.reactor_timer_timout)
